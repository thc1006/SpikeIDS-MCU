# SpikeIDS-MCU — research results under revalidation

Current status, 2026-09-22 07:08 Asia/Taipei: the bounded manuscript-artifact
acceptance publisher actually exited0 (session17280), followed by root
metadata postcheck72646 exit0 over4111 original commitments. Content review,
genuine PDF build/replay, independent retention and both agents' nine-page
visual checks have passed. The readable [audited PDF](results/v5_paper_continued_review_20260922_sQdjK1/pdf_actual_02/build/main.pdf)
is **not** venue/publication, release or hardware acceptance. Independent actual
postreview76589 also exited0. A new full-model/result copy-only tool is under
implementation and adversarial review; no genuine copy has yet been executed.
See the [current phase index](results/v5_paper_continued_review_20260922_sQdjK1/STATUS.md)
and [manuscript actual-exit receipt](results/v5_paper_continued_review_20260922_sQdjK1/ROOT_MANUSCRIPT_ACCEPTANCE_ACTUAL_EXIT.json).
The earlier failures below remain failures; do not rerun them or use the old
package/archive tools on the current mixed-origin evidence chain.

> The material below is a retained legacy README, **not accepted research
> results**. The previous caches contain cross-split exact-input overlap;
> earlier accuracy, equivalence, T=1, deployment, latency and energy claims
> must not be cited as verified v5 findings. The old PDFs and result files
> are not a current release. The rejected r1 run has been stopped and retained
> for engineering audit only. The corrected r6-recovery1 neural/statistics chain
> completed on 2026-09-21 at 16:19 Asia/Taipei (440 fits and evaluated replicas).
> The bounded neural post-run review passed. The additional 84-fit tree baseline
> completed at 23:39 and passed post-run acceptance at 23:52 on 2026-09-21.
> Paper/hardware/release work is not accepted. None of this
> restores the legacy claims below.
> On 2026-09-22 the first neural export service stopped with observed exit 1
> at attempt 3 because its consumer conflated distribution and runtime Torch
> version strings. Two earlier numerical negatives remain negatives; 19 entries
> were unexecuted. A separately reviewed, frozen continuation was observed complete
> by 03:43 Asia/Taipei, with actual owner exit 0, retaining the original three and executing
> only the remaining 19. FP32 passed 4/11 and QDQ 1/11; 17 negatives remain.
> Its final preflight passed 901 regression, 370 focused and one real telemetry
> test. The real lifecycle post-audit subsequently passed at 03:54, with both
> auditor and external observer actually exiting 0 (session 8544), checking
> 3,185 held files and all 39 worker executions. By 04:02 all 22 fresh
> same-validator numerical replays also matched, with actual outer/child exit 0
> (session 59443); this is not a third independent numerical implementation.
> At 04:26 the bounded export evidence/adjudication phase passed acceptance
> (actual publisher exit 0, session 78173, 3,314 held evidence files).
> Numerical negatives must not be promoted to passes or general INT8/NPU
> deployment claims. By 05:04 a reviewed read-only renderer produced the real
> candidate at `results/v5_paper_continued_review_20260922_sQdjK1/candidate_actual_01/paper/`
> (actual session 55545 exit 0; post-exit check 50461 exit 0, 3,318 held inputs).
> It passed 22 author, 57 lifecycle, 36 scientific and 3 template controls;
> concrete draft failures remain archived with their fixes. The original live
> manuscript was not overwritten. Independent actual-content review passed.
> At 05:36 the first genuine PDF owner had failed (session 68301, actual exit 1):
> four TeX builds produced matching PDF/text, but a retained callback invoked
> the resource monitor after its stream had closed. Earlier fake-monitor tests
> missed that lifecycle. The failed stage and c8 source remain immutable;
> additive v2 repair and independent adversarial regressions are in progress.
> No PDF phase acceptance or publication is claimed; do not promote that output.
> This is not manuscript/publication/release or hardware acceptance.
> See the handoff for exact records.
> Do not restart or overwrite the failed export run or its registry.
>
> Current workflow: [v5 README](spikeids_v5/README.md),
> [handoff](spikeids_v5/CODEX_HANDOFF.md), and
> [adversarial acceptance gates](spikeids_v5/audit/ADVERSARIAL_REVIEW_20260921.md).

## Historical README (superseded; retained pending recoverable archival)

Original title: SNN-IDS: Sub-Millijoule Intrusion Detection on the STM32N6 Neural-ART NPU
