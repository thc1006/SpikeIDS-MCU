"""
UNSW-NB15 QCFS L=4 multi-seed experiment (GLOBECOM gap audit fix).

Mirrors experiment_unsw.py's ReLU pipeline but substitutes IDS_MLP_QCFS(L=4).
Needed to validate T=1 SNN≡INT8 ANN equivalence on a second dataset.

Output: results/unsw_qcfs_multiseed.json
"""
...
def _train_single(seed, L, X_train, y_train, X_test, y_test,
    ...
    if seed == 0:
        torch.save(best_state, MODEL_DIR / f"unsw_qcfs_L{L}_best.pth")
    return metrics
...
def main():
    ap = argparse.ArgumentParser(description="UNSW-NB15 QCFS multi-seed")
    ap.add_argument("--seeds", type=int, nargs="+", default=list(range(10)))
    ap.add_argument("--epochs", type=int, default=80)
    ap.add_argument("--L", type=int, default=4)
    ap.add_argument("--output", type=str, default="unsw_qcfs_multiseed.json")
    args = ap.parse_args()

    print("=" * 70)
    print(f"UNSW-NB15 QCFS L={args.L} multi-seed ({len(args.seeds)} seeds)")
    print("=" * 70)
    ...
    result = {
        "experiment": f"UNSW-NB15 QCFS L={args.L} multi-seed",
        "dataset": "unsw_nb15",
