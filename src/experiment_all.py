"""COMPLETE-VERSION SCAFFOLD — all four datasets + TinyCNN (extends experiment_multiseed(1).py).

Leakage-safe, audited multi-seed ReLU/QCFS/TinyCNN experiment (FP32, one device).
This is a SCAFFOLD copied from experiment_multiseed(1).py. The NSL-KDD path is fully
implemented and verified; UNSW / CICIDS2017 / IoT-23 are PLACEHOLDERS: prepare_data raises
NotImplementedError with a precise per-dataset requirement prompt (see _DATASET_SPECS). A
Codex agent completes each dataset by mirroring the NSL path with FIT-ONLY preprocessing.

TODO(codex) checklist:
  1. Implement prepare_data for unsw / cicids2017 / iot23 (fit-only preprocessing; see the
     NotImplementedError message + _DATASET_SPECS for exact loaders, categorical columns,
     class lists, and Inf/NaN handling). NEVER fit any scaler/encoder on val or test.
  2. TinyCNN (`--model cnn`) is wired into build_model; run it for nslkdd/unsw/cicids2017 only.
  3. Budgets (CLI, per dataset): iot23 = --epochs 40 --batch-size 1024; others = 80 / 512.
  4. Smoke EACH dataset (`--seeds 0 --model relu --epochs 3`) before any full run.
  5. Write scripts/assemble_from_leakagesafe.py to map this file's rich output schema onto the
     pipeline's expected files (see codex_handoff/README.md §12b).

Requires the project's config.py, models.py and metrics.py. No network access is performed.
"""
...
def build_model(models: Any, kind: str, seed: int, input_dim: int, classes: int,
    ...
    if kind == "relu":
        ...
        # TinyCNN baseline (NPU-eligible Conv2D 1x3). forward() reshapes (B,F)->(B,1,1,F).
        model = models.TinyCNN_IDS(input_dim=input_dim, num_classes=classes)
    else:
...
# Precise per-dataset requirements for the SCAFFOLD placeholders (Codex fills prepare_data).
_DATASET_SPECS = {
    "unsw": (
        "UNSW-NB15, 10-class. Raw: data/UNSW_NB15_training-set.parquet (175341 rows) + "
        "testing-set.parquet (82332); the project uses training-set as TRAIN (the larger file). "
        "Reuse data_loaders.load_unsw_raw for the DataFrames. Categorical cols = "
        "['proto','service','state'] (CONFIRM against data_loaders.preprocess_unsw). Label col "
        "= 'attack_cat' (10 classes incl 'Normal'); build a FIXED class list. Drop id/label; "
        "coerce numerics; ensure no Inf/NaN. Budget 80ep / batch 512."
    ),
    "cicids2017": (
        "CICIDS2017, 15-class. Raw via experiment_cicids2017.load_cicids2017(grouped=False). "
        "~69 numeric flow features, NO categorical columns (cat_cols=[]). REPLACE +/-Inf and "
        "NaN before scaling. n_train~2.26M / n_test~566k. Reuse experiment_cicids2017's split "
        "logic but move the StandardScaler fit to the fit-split ONLY. Budget 80ep / batch 512."
    ),
    "iot23": (
        "IoT-23, 5-class (Benign, C&C, DDoS, Okiru, PortScan). Raw via "
        "experiment_iot23.load_iot23(binary=False). 13 features. AUDIT experiment_iot23 ~L127 "
        "`enc.fit(df[col])` — it may fit on the FULL df (leakage); here fit categorical encoders "
        "on train.iloc[fit] ONLY. Budget 40ep / batch 1024 (NOT 80/512)."
    ),
}
...
def prepare_data(args: argparse.Namespace, config: Any) -> Data:
    if args.dataset != "nslkdd":
        raise NotImplementedError(
            "[SCAFFOLD] dataset '%s' is not implemented yet — implement it in prepare_data.\n\n"
            "REQUIREMENT: %s\n\n"
            "BLUEPRINT (mirror the NSL-KDD path below, keeping every leakage-safe guarantee):\n"
            "  1) load raw TRAIN and TEST DataFrames (loader named above);\n"
            "  2) LabelEncoder over the FIXED class list; verify TEST contains all classes;\n"
            "  3) fit/val split: split_indices(y_train, args.val_fraction, args.split_seed, n_classes);\n"
            "  4) fit a preprocessor on train.iloc[fit] ONLY (NEVER val/test) — same idea as "
            "TrainOnlyPreprocessor but with THIS dataset's categorical columns;\n"
            "  5) transform fit / val / test with that fitted preprocessor;\n"
            "  6) class weights from the FIT split only;\n"
            "  7) return Data(...) with the same metadata + *_sha256 fields as the NSL path.\n"
            "Then smoke it: --seeds 0 --model relu --epochs 3 --dataset %s."
            % (args.dataset, _DATASET_SPECS[args.dataset], args.dataset))
...
def make_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--model", choices=("relu", "qcfs", "cnn", "both"), default="both",
                   help="'both'=relu+qcfs; run TinyCNN as its own '--model cnn' (nslkdd/unsw/cicids2017)")
    p.add_argument("--dataset", choices=("nslkdd", "unsw", "cicids2017", "iot23"), default="nslkdd",
                   help="nslkdd is IMPLEMENTED; unsw/cicids2017/iot23 are SCAFFOLD placeholders (see prepare_data / _DATASET_SPECS)")
