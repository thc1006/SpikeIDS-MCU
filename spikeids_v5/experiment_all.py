...
def prepare_data(args):
    meta, arrays = open_cache(args.cache)
    require(meta["dataset"] == args.dataset, "Prepared cache belongs to another dataset")
    prep = load_json(args.cache / "preprocessing.json")
    y_fit = arrays["y_fit"]
    counts = np.bincount(y_fit, minlength=len(meta["class_names"])).astype(np.float32)
    require((counts > 0).all(), "Fit split is missing a declared class")
    weights = 1. / counts
    weights = (weights / weights.sum() * len(counts)).astype(np.float32)
    tensors = [torch.from_numpy(arrays[name]) for name in ("x_fit", "y_fit", "x_validation", "y_validation", "x_test", "y_test")]
    return Data(*tensors, meta["class_names"], torch.from_numpy(weights),
                {**meta, "preprocessor": prep}, {s: arrays["ids_"+s] for s in ("fit","validation","test")})
...
def main():
    ...
    with output_lock(out.with_suffix(".lock")):
        ...
        protocol = {k: getattr(args,k) for k in ("dataset","model","seeds","epochs","batch_size","eval_batch_size",
                     ...
                     "device","optimizer","threads","data_placement","compile")}
        protocol.update(precision="IEEE FP32; AMP/TF32 off", selection="best validation macro recall; earliest exact tie",
        ...
        if manifest_path.exists():
            ...
            previous = {"fingerprint": fingerprint, "binding": binding, "test_sessions_started": 0}
        ...
        output = {"schema":SCHEMA,"dataset":args.dataset,"kind":args.model,"protocol":protocol,
                  ...
                  "n_official_train":data.metadata["n_official_train"],"scope":data.metadata["scope"],
        ...
        for seed in args.seeds:
            ...
            if args.stage=="evaluate":
                state=load_checkpoint(cp,fingerprint)
