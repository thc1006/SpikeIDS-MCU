...
def main():
    ...
    with file_lock(a.run_dir/"finalize.lock"):
        ...
        write_text(path,macro_text(expected_macros(plan,results,d,e)))
        ...
        if a.build:
            ...
            report=check(a.run_dir,a.paper_dir,strict=True)
            write_json(a.run_dir/"paper_numeric_check.json",seal(report))
            subprocess.run(["latexmk","-pdf","-interaction=nonstopmode","-halt-on-error","-cd",
                            str((a.paper_dir/"main.tex").resolve())],check=True)
            print("LaTeX build succeeded after numerical checks; scientific prose still requires review")
