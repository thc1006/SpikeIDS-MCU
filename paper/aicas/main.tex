...
120--138\,KB.
Compared with recent MCU-class deployments on the STM32F7
(31\,ms, 7.86\,mJ) and Raspberry Pi~3B+ (27\,ms), our NPU path
achieves 59--107$\times$ lower latency and 114--179$\times$
lower energy than the STM32F7.
...
Diab~\cite{diab2025rpi} & RPi~3B+ & multi & 95.3\% & 27\,ms & ${\sim}$6.75\,mJ$^\ddagger$ & None \\
\midrule
...
\multicolumn{7}{l}{\scriptsize $^\ddagger$Estimated from reported power and latency.}
\end{tabular}%
}
\end{table}
...
a graph of \texttt{Gemm}+\texttt{Relu} operators only (${\sim}$110K parameters).
...
% Row 4: U2R
\foreach \x/\v [count=\i from 0] in {24.0/24.0, 0.0/0.0, 50.9/50.9, 6.6/6.6, 18.6/18.6} {
...
\end{tikzpicture}
\caption{NSL-KDD confusion matrix (\%, 10-seed mean, ReLU MLP).
...
\textbf{114--179$\times$} less than Chehade et~al.'s 7.86\,mJ on
...
forcing CPU fallback with 17.4\% latency overhead.
...
commodity MCU NPU, achieving 2.7--4.2$\times$ CPU speedup and over two orders of
magnitude energy reduction versus CPU-only MCU deployments.
