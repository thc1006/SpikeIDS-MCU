...
$64.75{\pm}0.61$\% overall accuracy.
...
achieves 60--100$\times$ lower latency and over 100$\times$
lower energy per inference.
To our knowledge, this is the first IDS deployment on an ARM
Cortex-M NPU and the first non-vision workload on the Neural-ART
accelerator.
\end{abstract}
...
We put this theory to the test on the STM32N6570-DK (ARM Cortex-M55
...
Our contributions:
\begin{itemize}
  \item The first IDS classifier deployment on an ARM Cortex-M NPU,
...
Separately, several GPU-based SNN-IDS systems have been
proposed~\cite{roy2019towards}: convolutional SNNs~\cite{nature2024csnn},
event-driven SNN~\cite{ieee2025event}, and transformer-spiking
...
Diab~\cite{diab2025rpi} & RPi~3B+ & multi & 95.3\% & 27\,ms & ${\sim}$6.8\,mJ & None \\
\midrule
\textbf{This work} & \textbf{STM32N6} & \textbf{5/10} & \textbf{78.9\%} & \textbf{0.29\,ms} & \textbf{44\,\textmu J} & \textbf{NPU} \\
\bottomrule
...
because each \texttt{Floor} triggers a
\texttt{DequantizeLinear}$\to$\texttt{Floor(float)}$\to$\texttt{QuantizeLinear}
CPU round-trip.
...
Fig.~\ref{fig:cm} shows the NSL-KDD confusion matrix.
The dominant error pattern is R2L$\to$Normal (64.4\% of R2L)
and U2R$\to$Probe (50.9\% of U2R), both caused by severe class imbalance
(R2L: 2,199 train / 2,754 test; U2R: 52 train / 200 test).
Normal and DoS, which dominate the training set, achieve $>$99\% and
$>$86\% recall respectively.

\begin{figure}[t]
\centering
\begin{tikzpicture}[scale=0.58]
% Row 0: Normal
\foreach \x/\v [count=\i from 0] in {99.8/99.8, 0.0/0.0, 0.2/0.2, 0.0/0.0, 0.0/0.0} {
...
% Row 1: DoS
\foreach \x/\v [count=\i from 0] in {5.2/5.2, 86.7/86.7, 0.9/0.9, 7.2/7.2, 0.0/0.0} {
...
% Row 2: R2L
\foreach \x/\v [count=\i from 0] in {64.4/64.4, 0.7/0.7, 25.0/25.0, 9.5/9.5, 0.4/0.4} {
...
% Row 3: Probe
\foreach \x/\v [count=\i from 0] in {3.5/3.5, 6.3/6.3, 1.0/1.0, 88.9/88.9, 0.3/0.3} {
...
% Row 4: U2R
\foreach \x/\v [count=\i from 0] in {24.0/24.0, 1.5/1.5, 5.0/5.0, 50.9/50.9, 18.5/18.5} {
...
% Labels
\foreach \lab/\y in {Norm/0, DoS/1, R2L/2, Probe/3, U2R/4} {
...
\end{tikzpicture}
\caption{NSL-KDD confusion matrix (\%, ReLU INT8, best seed).
...
FP32 = CPU-only baseline (0 HW epochs).}
\label{tab:npu}
\small
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}llcccccc@{}}
\toprule
\textbf{Model} & \textbf{Dataset} & \textbf{Time} & \textbf{Energy} & \textbf{HW} & \textbf{SW} & \textbf{Flash} & \textbf{RAM} \\
 & & \textbf{(ms)} & \textbf{(\textmu J)} & & & \textbf{(KB)} & \textbf{(KB)} \\
\midrule
ReLU FP32 & NSL-KDD & 1.24 & 186 & 0 & 11 & 466.4 & 2.17 \\
ReLU INT8 & NSL-KDD & 0.46 & 69 & 5 & 2 & 137.7 & 1.25 \\
\midrule
ReLU FP32 & UNSW-NB15 & 1.23 & 185 & 0 & 11 & 461.9 & 2.14 \\
ReLU INT8 & UNSW-NB15 & \textbf{0.29} & \textbf{44} & 4 & 0 & 120.6 & 0.50 \\
\midrule
QCFS INT8 & NSL-KDD & 0.54 & 81 & 13 & 14 & 138.0 & 2.00 \\
\bottomrule
\end{tabular}%
}
\end{table}

Table~\ref{tab:npu} presents hardware measurements.
...
\begin{axis}[
    ...
    symbolic x coords={Chehade,Diab,CPU,NPU-NSL,NPU-UNSW},
    xtick=data,
...
\end{axis}
\end{tikzpicture}
\caption{Energy per inference comparison (log scale). NPU deployment
achieves over 100$\times$ reduction versus CPU-only MCU baselines.
CPU = ReLU FP32 on Cortex-M55; NPU = ReLU INT8 on Neural-ART.
...
The 0.29--0.46\,ms NPU inference time fits within a 1\,ms RTOS tick.
Because the Cortex-M55 is idle during NPU execution, concurrent tasks
(Ethernet DMA, feature extraction, display refresh) can proceed
without deadline misses that would occur during the 1.2\,ms CPU-only
path. Validating this with a \textmu T-Kernel~3.0 integration is
planned future work.
...
All code and trained models will be publicly released.
