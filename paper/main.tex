...
on an ARM Cortex-M NPU and the first publicly documented empirical validation of $T{=}1$
...
\textbf{This work} & 2026 & NSL-KDD, UNSW & 5/10-cls & MLP (T=1 SNN eq.) & STM32N6 & MCU + NPU & Yes & Yes & No & 78.9/64.8\% \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\begin{table}[t]
\centering
\caption{INT8 quantization ablation on NSL-KDD. FP32 baseline: 76.45\% (seed 42, used for ONNX export).
...
adding 0.08\,ms (+17.4\%) over ReLU INT8.
...
Combined with 17.4\% latency overhead from \texttt{Floor} CPU fallback,
...
the 10-class problem with extreme imbalance (Worms: 44 test samples)
...
NPU support, forcing CPU fallback with 17.4\% latency overhead.
