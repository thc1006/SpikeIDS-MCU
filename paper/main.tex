...
On UNSW-NB15 (10-class), per-class performance varies widely:
Generic (F1$=$97.9$\pm$0.2\%) and Reconnaissance (75.1$\pm$2.7\%)
perform well, but extreme minority classes collapse ---
Worms (44~test samples, F1$=$9.0\%), Analysis (F1$=$2.6\%),
and Backdoor (F1$=$10.6\%).
Normal achieves high precision (98.8\%) but only 57.6\% recall,
indicating that the model under-predicts the majority class in favor
of attack categories due to inverse-frequency weighting.
Full per-class results and confusion matrices are available in the
code repository.
