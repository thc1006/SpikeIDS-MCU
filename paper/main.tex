...
occupying 120.6--137.7\,KB Flash and 0.5--1.25\,KB RAM.
A comparison with QCFS activation reveals that the \texttt{Floor}
operator falls back to CPU on this NPU, adding 17.6\% latency.
Tree-based baselines (Random Forest, XGBoost) confirm that the MLP
offers the best accuracy on NSL-KDD while being the only model eligible
for NPU acceleration.
To our knowledge, this is the first IDS deployment on an ARM Cortex-M
NPU and the first empirical validation of $T{=}1$ SNN--ANN equivalence
on commercial NPU silicon.
\end{abstract}
...
More recently, Chen et al.~\cite{chen2025one} (preprint) and
...
The classifier infers in 0.29--0.46\,ms with a memory footprint under 140\,KB.
We emphasize that this paper evaluates the \emph{classifier inference}
stage on pre-extracted flow-level features; a complete end-to-end IDS
pipeline (packet capture, flow aggregation, feature extraction) is left
to future work.

Our contributions:
\begin{itemize}
  \item We present, to our knowledge, the first IDS classifier deployment
    on an ARM Cortex-M NPU (Neural-ART). Prior MCU-class IDS work used
  ...
  \item We provide the first empirical validation of the $T{=}1$ SNN--ANN
...
deployment on physical hardware. ``Timing'' and ``Power'' indicate measured
(not estimated) values.}
\label{tab:related}
\small
\begin{tabular}{@{}llllllllll@{}}
\toprule
\textbf{Paper} & \textbf{Year} & \textbf{Dataset} & \textbf{Model} & \textbf{Hardware} & \textbf{HW Type} & \textbf{On-chip} & \textbf{Timing} & \textbf{Power} & \textbf{Acc.} \\
\midrule
Wang~\cite{nature2024csnn} & 2024 & NSL-KDD & Conv SNN & GPU & GPU & No & No & No & 94.7\% \\
Zahm~\cite{zahm2024csiac} & 2024 & UNSW-NB15 & SNN & Akida AKD1000 & Neurom.\ ASIC & Yes & Yes & Yes & 98.4\% \\
Prajwal.~\cite{ieee2025event} & 2025 & NSL-KDD & Event SNN & GPU & GPU & No & No & No & 95.4\% \\
Mustafa~\cite{pmc2025hybrid} & 2025 & UNSW-NB15 & RNN-SNN & GPU & GPU & No & No & No & 99.7\% \\
Karthik~\cite{nature2026transformer} & 2026 & NSL-KDD & Transf.-SNN & GPU & GPU & No & No & No & 99.2\% \\
Ngo~\cite{ngo2022hhnids} & 2022 & UNSW-NB15 & MLP & MAX78000 & AI-spec.\ MCU & Yes & Yes & Yes & 98.6\% \\
\textbf{This work} & 2026 & NSL-KDD, UNSW & MLP (T=1 SNN eq.) & STM32N6 & MCU + NPU & Yes & Yes & No & 78.9\% \\
\bottomrule
\end{tabular}
\end{table*}
...
for $T{=}1$ conversion.
Chen et al.~\cite{chen2025one} (preprint) demonstrated
that the $T{=}1$ threshold-and-fire mechanism reduces to a
clamp, making the forward pass approximately equivalent to an INT8
quantized ANN under matched quantization grids.
...
of the $T{=}1$ equivalence on commercial NPU silicon.
...
To our knowledge, no prior work has deployed an IDS on any general-purpose
...
operation~\cite{bu2022optimal, jiang2023unified, chen2025one, bu2025inference}.
...
\node[reslbl, fill=green!8, rounded corners=2pt,
      above=0.35cm of npu]
  {4--5 HW + 0--2 SW\\\textbf{0.29--0.46\,ms}};
...
On NSL-KDD (41 features, 5 classes), ReLU INT8 completes in 0.46\,ms
with 5~HW, 1~hybrid, and 2~SW epochs.
On UNSW-NB15 (34 features, 10 classes), the smaller input dimension
yields 100\% NPU execution (4~HW epochs, zero SW fallback) at
\textbf{0.29\,ms} (3,397~inferences/sec), 35\% faster than NSL-KDD.

QCFS INT8 runs \texttt{Gemm} on the NPU but each \texttt{Floor} triggers
a CPU fallback. The compiler inserts \mbox{\texttt{DequantizeLinear}}
$\to$ \mbox{\texttt{Floor(float)}} $\to$ \mbox{\texttt{QuantizeLinear}}
at every QCFS layer, adding 0.08\,ms (+17.6\%).
QCFS FP32 runs entirely on CPU (0 HW epochs, 20 SW epochs), 3.1$\times$
slower than ReLU INT8.

The \texttt{Floor} operator is absent from ST's published
NPU operator list~\cite{st_neural_art}. Our measurements pin down the
cost of the resulting SW\_FLOAT fallback at 0.08\,ms per inference.
...
Although individual neurons show moderate quantization noise
(cosine similarity 0.65--0.68), the downstream effect on classification
decisions is minimal: 99\% of test samples receive the same predicted
class from FP32 and INT8 models.
...
\subsection{QCFS: Better Theory, Worse Hardware Fit}

With multi-seed evaluation, ReLU outperforms QCFS on NSL-KDD
($p{=}0.037$). The single-seed observation that QCFS was superior
was a statistical artifact. Combined with the 17.6\% latency penalty
from \texttt{Floor} CPU fallback, QCFS offers no advantage on this
NPU target.
...
Fifth, UNSW-NB15 accuracy (64.75\%) is below tree baselines;
the MLP architecture likely needs hyperparameter tuning or feature
...
accuracy on NSL-KDD (0.46\,ms) and $64.75{\pm}0.61$\% on UNSW-NB15
(0.29\,ms, 100\% NPU execution).
A 24-configuration quantization ablation shows INT8 accuracy varies
by less than 1\,pp across calibration choices.
Layer-wise analysis shows 99\% prediction agreement between
