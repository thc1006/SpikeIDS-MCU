#ifndef S6_TRACE_N6_MAILBOX_H
#define S6_TRACE_N6_MAILBOX_H
#include <stddef.h>
#include <stdint.h>

#define S6_MAGIC UINT32_C(0x54364E36)
#define S6_PROTOCOL 1u
#define S6_TRACE_ADDRESS UINT32_C(0x340F8200)
#define S6_TRACE_BYTES 6556u
...
#define S6_DEPLOYMENT_TAG UINT32_C(0x54523031)
...
enum { S6_E_PLATFORM=-1, S6_E_COMMAND=-2, S6_E_INPUT=-3, S6_E_CHANGED=-4,
       S6_E_LAYOUT=-5, S6_E_OUTPUT=-6, S6_E_CACHE=-7, S6_E_TRACE=-8 };
...
typedef struct {
    ...
    uint8_t tail_padding[2]; /* Preserve both complete 65-byte hash strings. */
    uint32_t trace_address, trace_bytes, trace_run_id;
    int32_t trace_status, trace_bind_status;
    uint32_t trace_complete, trace_error;
} s6_mailbox_t;
...
_Static_assert(offsetof(s6_mailbox_t, trace_address) == 484, "Trace tail offset changed");
_Static_assert(offsetof(s6_mailbox_t, trace_error) == 508, "Trace tail end changed");
