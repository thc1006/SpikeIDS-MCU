...
volatile npu_trace_log_t g_npu_trace __attribute__((section(".npu_trace"), aligned(32)));
static npu_trace_context_t trace_context;
_Static_assert(sizeof(npu_trace_log_t) == S6_TRACE_BYTES, "Trace ABI changed");
...
/* Raw DWT callback observations only: enable/counting/frequency are not attested. */
static uint32_t trace_counter(void *unused) { (void)unused; return DWT->CYCCNT; }
static void trace_snapshot(void)
{
    g_mailbox.trace_run_id=g_npu_trace.run_id;
    g_mailbox.trace_complete=g_npu_trace.complete;
    g_mailbox.trace_error=g_npu_trace.error;
    __DMB();
}
...
int main(void)
{
    ...
    memset((void *)&g_npu_trace, 0, sizeof g_npu_trace);
    g_mailbox.trace_address=S6_TRACE_ADDRESS;
    g_mailbox.trace_bytes=sizeof g_npu_trace;
    g_mailbox.trace_status=INT32_MIN;
    g_mailbox.trace_bind_status=INT32_MIN;
    ...
    g_mailbox.trace_bind_status=npu_trace_bind(network,&trace_context,&g_npu_trace,trace_counter,0);
    trace_snapshot();
    if (g_mailbox.trace_bind_status!=STAI_SUCCESS) park_error(S6_E_TRACE);
    ...
    for (;;) {
        ...
        /* begin clears the previous log: the host MUST retain it before committing this request. */
        g_mailbox.trace_status=npu_trace_begin(&trace_context,seq);
        trace_snapshot();
        if (g_mailbox.trace_status!=0) park_error(S6_E_TRACE);
        ...
        g_mailbox.run_cycles=end-begin; /* Instrumented mixed-run CPU window, not NPU cycles or accepted performance */
        /* Always finalize the actual sync return before any error path can park.
         * finish preserves a partial/error trace; callback failure cannot stop hardware. */
        g_mailbox.trace_status=npu_trace_finish(&trace_context,rc);
        trace_snapshot();
        g_mailbox.api_status[4]=(int32_t)rc;
        stai_return_code error_rc=stai_nsl_qcfs_seed0_get_error(network);
        g_mailbox.api_status[5]=(int32_t)error_rc;
        memcpy(output_copy,out[0],20);
        for (unsigned i=0; i<5; ++i) g_mailbox.output_words[i]=output_copy[i];
        g_mailbox.output_count=5; /* Raw output bytes are retained even when the runtime failed; not accepted logits. */
        if (rc!=STAI_SUCCESS || error_rc!=STAI_SUCCESS) park_error(0);
