#!/usr/bin/env python3
"""Additive callback-instrumented SRAM diagnostic build; no hardware or regeneration."""
import argparse
import hashlib
import json
import os
...
GEN = VENDOR / "generate"
ORIGINAL = HERE.parent / "firmware_sram"
TRACE = HERE.parent / "npu_trace"
FROZEN = {
    ORIGINAL/"build.py": "37194ebe0a164873310855e11ec911f8acab468e1728e1c382addf31808d2872",
    ORIGINAL/"main.c": "7af7972cc89b2137a2a6d7cee80ccdd69209d77c33f4ce0225863869bee6bf94",
    ORIGINAL/"startup.c": "cdfafb4fd5cf4d55123358fab1396d5bb9afa18a60ba5eb0a62f7bf1b3ce9160",
    ORIGINAL/"syscalls.c": "e1ce7b80b8e3e64c9b0d888674e0c68e1a4cd6197450b0b1bc07eff78b934f6a",
    ORIGINAL/"mailbox.h": "29f9f4fd7d96e3cd3ec14dec99d14dad7353f8cca8eef421bb9a7fd5ca434e78",
    ORIGINAL/"linker.ld": "b2179f2bbd0cf749998b68185f8b8a6918dd06a57d9c447449a4f0c3fafacc38",
    ORIGINAL/"ABI.json": "af5fc1970e128ac548a856838b53ba585f66dddcb67602d1b151f573efcfd8b1",
    TRACE/"npu_trace.c": "c1125ead0369ef7666e06568f3f0c2ac9efb955eaa57369402aa7c038db6bcbf",
    TRACE/"npu_trace.h": "35d66df9a39d43433144366979bd6bc46be0468d9e08b4f1f401554250fd9957",
    TRACE/"npu_trace_bind.c": "e16a82728e5bd13093763017314ed7c1f35044fde1b6a4a0dd40309fc3412bef",
}
...
INCLUDES = [HERE, TRACE, GEN, ST/"Inc", ST/"Npu/ll_aton", ST/"Npu/Devices/STM32N6xx",
...
LIMITS = {"board_ready": False, "board_validated": False, "energy_measured": False,
          ...
          "independent_hardware_proof": False, "performance_accepted": False,
          "trace_is_diagnostic_only": True}
...
def frozen_pins():
    ...
    for path,expected in FROZEN.items():
        current=hold(held,path)
        require(current["sha256"]==expected, "Frozen source identity changed")
    for name in ("startup.c","syscalls.c"):
        current=hold(held,HERE/name)
        require(current["sha256"]==FROZEN[ORIGINAL/name], "Unchanged startup/syscalls copy differs")
    bookend(held)
    return held
...
def elf_layout(raw):
    ...
    require(len(segments)==5,"Expected RX/data/stack/mailbox/trace segments")
    code,data,stack,mail,trace=segments
    ...
    require(trace["address"]==0x340F8200 and trace["memory_bytes"]==6556 and
            trace["file_bytes"]==0 and trace["flags"]==6,"Trace NOLOAD segment")
    require(mail["address"]+mail["memory_bytes"]==trace["address"] and
            trace["address"]+trace["memory_bytes"]<=0x34180000,"Trace disjoint extent")
...
def run(out):
    out=Path(out)
    require(out.is_absolute() and out==out.resolve() and out.parent==HERE, "Use a fresh direct child of firmware_sram_trace")
    ...
    for path,current in frozen_pins().items():
        require(path not in held or held[path]==current,"Direct input changed before frozen-source check")
        held[path]=current
    ...
    sources=[HERE/"startup.c",HERE/"main.c",HERE/"syscalls.c",GEN/"nsl_qcfs_seed0.c",GEN/"stai_nsl_qcfs_seed0.c"]+[TRACE/"npu_trace.c",TRACE/"npu_trace_bind.c"]+runtime+devices+hal
    ...
    try:
        write("INTENT.json",encoded({"schema":1,"kind":"n6_sram_trace_offline_firmware_build_intent","direct_pins":held,**LIMITS}))
        ...
        elf=out/"n6_sram_trace.elf"
        command("link",[tools["gcc"],*FLAGS,"-T",HERE/"linker.ld","-nostartfiles","-Wl,--gc-sections",
                        "-Wl,--fatal-warnings","-Wl,--print-memory-usage","-Wl,-Map="+str(out/"n6_sram_trace.map"),
        ...
        require(all(name in symbols for name in ("g_mailbox","Reset_Handler","g_npu_trace",
                "npu_trace_bind","npu_trace_begin","npu_trace_callback","npu_trace_finish")),"Missing protocol/startup/trace symbols")
        command("disassembly",[tools["objdump"],"-d",elf])
        layout=elf_layout(elf.read_bytes())
        command("binary",[tools["objcopy"],"-O","binary",elf,out/"n6_sram_trace.bin"])
        ...
        report={"schema":1,"kind":"n6_sram_trace_offline_firmware_build","arm_compile_link_succeeded":True,
                ...
                "limits":["No FSBL/SRAM-clock/NPU/RIF/watchdog initialization supplied or verified.",
                          ...
                          "162 runtime callbacks include 160 epoch events plus sync DeInit/Init; not independent hardware proof.",
                          "Instrumented callback windows perturb runtime: neither performance nor energy acceptance.",
                          "Host must halt after matching completion/error and retain all 6556 trace bytes before the next request.",
