"""SM03 48-stop binding controls; no real hardware in tests."""
...
def modules():
    ...
    try:
        spec=importlib.util.spec_from_file_location('sm03_epoch_binding',HERE/'diagnose_epochs.py')
    ...
    c=f.Core();raw=bytearray(c.previous);struct.pack_into('<I',raw,124,0x534d3033)
...
def test_48_ordered_stops_one_commit_and_no_ambiguous_retry(modules,bad):
    ...
    call=lambda:d.observe(c,f.payload(),s,lambda b:rp.decode(b,m['protocol'].decode),'hardware',c.previous,
    ...
    if bad=='none':
        ...
        assert len([n for n in s.files if n.endswith('_activation.bin')])==48
    else:
