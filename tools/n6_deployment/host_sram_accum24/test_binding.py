...
spec=importlib.util.spec_from_file_location('sm05_launcher',HERE/'run.py')
...
def test_private_modules_do_not_change_old_identity(bound):
    ...
    assert rp.TAG==0x534d3035 and fixture.rp.TAG==0x534d3032
    assert sys.modules['runtime_protocol'] is fixture.rp
    assert controller.decode is rp.decode


@pytest.mark.parametrize('old_tag',[0x534d3031,0x534d3032,0x534d3033,0x534d3034])
...
def test_sm05_raw_fields_retained_old_host_rejects(bound,state):
    _,m,_,_,rp=bound
    raw=bytearray(fixture.message(state));struct.pack_into('<I',raw,124,0x534d3035)
...
def test_new_tag_does_not_bypass_existing_receipts(bound,word,value):
    _,m,_,_,rp=bound
    raw=bytearray(fixture.message());struct.pack_into('<I',raw,124,0x534d3035)
...
class Core(fixture.Core):
    def start_loaded_image(self,entry,msp):
        ...
        if entry<0x34100000:self.put(fixture.protocol.ADDRESS+124,struct.pack('<I',0x534d3035))
...
def test_full_flow_and_adversarial_gates(bound,bad):
    ...
    if bad=='none':
        ...
        assert struct.unpack_from('<I',bytes.fromhex(s.files['row_1023.json']['raw_mailbox_hex']),124)[0]==0x534d3035
    else:
...
def test_real_fixed_bundle_offline_only(bound):
    ...
    assert len(f['rows'])==1024 and old.BUILD==launcher.BUILD and rp.TAG==0x534d3035
