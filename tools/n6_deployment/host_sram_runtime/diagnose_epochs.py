"""One explicit diagnostic replay using hardware breakpoints, not parity acceptance.

No firmware recompilation, software breakpoint, Flash/PPK or tolerance change.
The previously completed negative run is not resumed as an accepted experiment.
"""
import argparse
import hashlib
import json
...
def observe(core,firmware,sink,decode,hw_type,expected_previous,*,clock=time.monotonic,sleep=time.sleep):
    ...
    try:
        ...
        w=decode(prior)
        require(prior==expected_previous and w[3]==5 and w[5]==w[6]==1024,
                'Not exact completed negative attempt 03')
        ...
        write(ADDRESS+20,struct.pack('<I',1025))
        for index,(name,address) in enumerate(POINTS):
            ...
            words=decode(raw)
            require(words[3]==4 and words[5:7]==(1025,1024) and words[8]==row_id
        ...
        while True:
            ...
            if w[3]==5 and w[6]==1025:break
            sleep(0.001)
        core.halt();final=core.read(ADDRESS,512);sink.raw('final_mailbox.bin',final)
        require(final==raw and w[5]==1025 and w[19]==row_id and final[128:292]==inputs,'Wrong final diagnostic response')
        require(w[11:19]==(0,)*8 and w[10]==5,'Diagnostic API failure')
        return dict(diagnostic_completed=True,original_row_id=row_id,sequence=1025,
...
def main():
    ...
    args=p.parse_args();modules,pins=launcher.dependencies();firmware=launcher.load_variant(modules)
    require(Path(launcher.__file__).resolve()==HERE/'run.py','Wrong launcher')
    pins[str(Path(__file__).resolve())]=launcher.sha(Path(__file__).resolve())
    previous=launcher.OLD.parents[2]/'results/n6_sram_validation_20260926_03/final_mailbox.bin'
    raw=previous.read_bytes()
    require(hashlib.sha256(raw).hexdigest()=='c870ecc0479f45f50334cd4bedfff3bf718d2b8884dfc6007dfb8f4775e2445e','Wrong negative attempt')
    if not args.execute_diagnostic:
        print(json.dumps(dict(offline_checked=True,hardware_accessed=False,points=POINTS)));return
    if args.output is None:p.error('--output required')
    sink=modules['orchestrate'].Store(args.output)
    sink.json('INTENT.json',dict(sources=pins,model_inputs=firmware['input_sha256'],points=POINTS,
        previous_mailbox_sha256=hashlib.sha256(raw).hexdigest(),kind='one_row_diagnostic_not_acceptance',
    ...
    try:
        ...
        with modules['pyocd_backend'].attach_exact_probe() as core:
            result=observe(core,firmware,sink,lambda b:rp.decode(b,modules['protocol'].decode),Target.BreakpointType.HW,raw)
        require(all(launcher.sha(Path(n))==d for n,d in pins.items()),'Sources changed')
