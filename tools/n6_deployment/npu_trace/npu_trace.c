...
_Static_assert(LL_ATON_RT_Callbacktype_PRE_START==0 && LL_ATON_RT_Callbacktype_POST_START==1 &&
               ...
               LL_ATON_RT_Callbacktype_NN_DeInit==5 && LL_ATON_RT_Callbacktype_NN_Init==4,
...
_Static_assert(sizeof(npu_trace_log_t)==6556, "fixed log size");
...
void npu_trace_callback(void *cookie, const stai_event_type event_type, const void *payload)
{
    ...
    row->sequence=slot;
    row->epoch_index=index<NPU_TRACE_EPOCHS ? index : UINT32_MAX;
    row->compiler_epoch=index<NPU_TRACE_EPOCHS ? epoch_numbers[index] : UINT32_MAX;
    ...
    ++log->stored;
    if (log->matched==NPU_TRACE_EPOCH_EVENTS) {
        /* STAI_MODE_SYNC resets the completed network before returning success.
         * RT_Reset_Network emits NN_DeInit(NULL), then NN_Init(NULL). */
        uint32_t expected=log->completed_lifecycle==0 ? LL_ATON_RT_Callbacktype_NN_DeInit : LL_ATON_RT_Callbacktype_NN_Init;
        if (log->completed_lifecycle>=2 || event_type<0 || (uint32_t)event_type!=expected) {
            (void)fail(ctx,NPU_TRACE_EVENT);return;
        }
        if (payload!=NULL) { (void)fail(ctx,NPU_TRACE_POINTER);return; }
        ++log->completed_lifecycle;
        return;
    }
...
int npu_trace_finish(npu_trace_context_t *ctx, stai_return_code runtime_result)
{
    ...
    if (log->matched!=NPU_TRACE_EPOCH_EVENTS || log->stored!=NPU_TRACE_EVENTS ||
