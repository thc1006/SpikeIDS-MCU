...
int main(void)
{
    setup();emit(160);CHECK(npu_trace_finish(&ctx,STAI_SUCCESS)==0);
    ...
    CHECK(npu_trace_begin(&ctx,2)==0);emit(160);CHECK(npu_trace_finish(&ctx,0)==0);++tests;
    ...
    setup();emit(160);npu_trace_callback(&ctx,0,&table[40]);failed(NPU_TRACE_OVERFLOW);
    CHECK(guarded.log.stored==160);CHECK(npu_trace_finish(&ctx,0)!=0);
    setup();emit(160);CHECK(npu_trace_finish(&ctx,STAI_ERROR_GENERIC)!=0);failed(NPU_TRACE_RUN_FAILED);
    setup();emit(160);table[40].wait_mask=1;CHECK(npu_trace_finish(&ctx,0)!=0);failed(NPU_TRACE_CHANGED);
    ...
    setup();emit(160);CHECK(npu_trace_finish(&ctx,0)==0);
    npu_trace_callback(&ctx,0,&table[0]);failed(NPU_TRACE_STATE);
    setup();clock_word=UINT32_MAX-10;emit(160);CHECK(npu_trace_finish(&ctx,0)==0);
    ...
    CHECK(npu_trace_begin(&ctx,1)==0);emit(160);CHECK(npu_trace_finish(&ctx,0)==0);
