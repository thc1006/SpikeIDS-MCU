"""Offline ARM stage build v3 (linker token whitespace; installed-tool hardlinks retained). No device transport or target commands.
...
def tool_snapshot(v,path):
    """Installed tools may have hardlinks. Retain their original nlink as identity."""
    v.canonical(path)
    fd=os.open(path,os.O_RDONLY|os.O_NOFOLLOW)
    try:
        before=v.stat_record(os.fstat(fd))
        v.require(stat.S_ISREG(before['mode']) and before['links']>=1,'Tool must be ordinary regular')
        digest=hashlib.sha256()
        while block:=os.read(fd,1024*1024):digest.update(block)
        v.require(before==v.stat_record(os.fstat(fd))==v.stat_record(path.lstat()),'Tool changed during read')
...
def build(output):
    ...
    inputs={str(p):v.snapshot(p) for p in [Path(__file__).resolve(),VENDOR,*HEADERS,
         *(HERE/n for n in ('platform_init.c','platform_init.h','main.c','startup.c','mailbox.h','linker_v2.ld','ABI.json'))]}
    ...
    for p in tools.values():
        tool_paths.add(str(p));inputs[str(p)]=tool_snapshot(v,p)
    def recheck(pins):
        v.recheck({n:p for n,p in pins.items() if n not in tool_paths})
        for name,pin in pins.items():
            if name in tool_paths:
                v.require(tool_snapshot(v,Path(name))==pin,'Installed tool changed: '+name)
        # Final stat pass across both domains after all hash reads.
        for name,pin in pins.items():
            v.require(v.stat_record(Path(name).lstat())=={k:x for k,x in pin.items() if k!='sha256'},
                      'Late input/tool change: '+name)
    ...
    try:
        ...
        recheck(inputs);owner.write('COMPILE_INPUTS.json',v.encoded(inputs))
        objects=[];object_pins={}
        for name in ('main','startup','platform_init'):
            obj=output/(name+'.o');call('compile_'+name,[*base,'-c',HERE/(name+'.c'),'-o',obj])
            objects.append(obj);object_pins[str(obj)]=v.snapshot(obj)
        recheck({**inputs,**object_pins})
        elf=output/'stage.elf'
        call('link',[tools['gcc'],*FLAGS,'-nostdlib','-nostartfiles','-nodefaultlibs','-T',HERE/'linker_v2.ld',
        ...
        recheck({**inputs,**object_pins,**owner.pins});owner.guard()
        ...
        result=dict(schema=1,kind='offline_secure_sram_platform_stage_build',compile_link_succeeded=True,
          actual_process_exit=None,source_pins=inputs,installed_tool_paths=sorted(tool_paths),artifact_pins_before_result=artifacts,abi=abi,
        ...
        recheck({**inputs,**owner.pins,**{str(output/n):p for n,p in artifacts['files'].items()}})
        v.require({p.name for p in output.iterdir()}==set(artifacts['files'])|{'RESULT.json'},'Unexpected output')
        recheck(inputs);owner.guard();return result
