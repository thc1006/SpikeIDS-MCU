"""SM06 fresh-init observer controls; all device calls are explicit fakes."""
...
def setup():
    ...
    try:d=load('sm06_diagnostic',HERE/'diagnose.py');engine=d.engine(rp)
    finally:
    ...
    try:f=load('sm06_observer_fakes',HERE.parent/'host_sram_runtime/test_diagnostic.py')
    finally:
    ...
    c=Core();raw=bytearray(fixture.message(3));struct.pack_into('<2I',raw,20,0,0);struct.pack_into('<I',raw,124,0x534d3036)
