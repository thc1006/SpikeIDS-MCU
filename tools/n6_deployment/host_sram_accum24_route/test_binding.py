...
spec=importlib.util.spec_from_file_location('sm06_launcher',HERE/'run.py')
...
def test_private_modules_do_not_change_old_identity(bound):
    ...
    assert rp.TAG==0x534d3036 and fixture.rp.TAG==0x534d3032
    assert sys.modules['runtime_protocol'] is fixture.rp
    assert controller.decode is rp.decode


@pytest.mark.parametrize('old_tag',[0x534d3031,0x534d3032,0x534d3033,0x534d3034,0x534d3035])
...
def test_sm06_raw_fields_retained_old_host_rejects(bound,state):
    _,m,_,_,rp=bound
    raw=bytearray(fixture.message(state));struct.pack_into('<I',raw,124,0x534d3036)
...
def test_new_tag_does_not_bypass_existing_receipts(bound,word,value):
    _,m,_,_,rp=bound
    raw=bytearray(fixture.message());struct.pack_into('<I',raw,124,0x534d3036)
...
class Core(fixture.Core):
    def start_loaded_image(self,entry,msp):
        ...
        if entry<0x34100000:self.put(fixture.protocol.ADDRESS+124,struct.pack('<I',0x534d3036))
...
def test_full_flow_and_adversarial_gates(bound,bad):
    ...
    if bad=='none':
        ...
        assert struct.unpack_from('<I',bytes.fromhex(s.files['row_1023.json']['raw_mailbox_hex']),124)[0]==0x534d3036
    else:
...
def test_real_fixed_bundle_offline_only(bound):
    ...
    assert len(f['rows'])==1024 and old.BUILD==launcher.BUILD and rp.TAG==0x534d3036


@pytest.mark.parametrize('bad',['none','irq','bus','receipt','retention'])
def test_init_only_never_commits_inference_and_never_claims_parity(bound,bad):
    _,_,_,controller,_=bound
    c,s=Core(),fixture.Sink();f,p=fixture.payloads();f['segments']=((0x34064000,bytes(256)),)
    if bad in ['irq','bus','receipt']:setattr(c,bad,0 if bad!='receipt' else False)
    if bad=='retention':
        original=s.raw
        def raw(name,value):
            if name=='diagnostic_ready.bin':raise OSError('disk')
            original(name,value)
        s.raw=raw
    call=lambda:launcher.initialize_only(c,f,p,s,nonce=31,modules=fixture.MODULES,controller=controller)
    if bad=='none':
        r=call();assert r['initialization_ready'] and r['completed_rows']==0
        assert not any(r[k] for k in ('inference_executed','full_logit_parity_passed','energy_measured','research_measurement_accepted'))
        assert 'PARITY.json' not in s.files
    else:
        with pytest.raises((ValueError,RuntimeError,OSError)):call()
    assert not any(a==fixture.protocol.ADDRESS+20 for a,_ in c.writes)
    assert not c.running and s.files['completion_state.json']['halt_error'] is None
