 ...
 * REQUIRED before platform_ack: reviewed secure execution/clock/RIF/XSPI2/NPU
 * setup, exact weights at 0x71000000, CPU D-cache disabled or noncacheable mailbox,
...
_Static_assert((V5_RUNTIME_IO_ADDRESS % 32) == 0, "Runtime I/O address is not aligned");

volatile v5_mailbox_t g_mailbox __attribute__((section(".mailbox"), aligned(32)));
...
static void park_error(int code)
{
    ...
    __DMB(); g_mailbox.state = V5_ERROR; __DSB();
    for (;;) __NOP();
}
...
int main(void)
{
    ...
    g_mailbox.state = V5_WAIT_PLATFORM;
    __DMB(); g_mailbox.magic = V5_MAGIC; __DSB();
    ...
    while (g_mailbox.platform_ack != V5_PLATFORM_ACK) __NOP();
    if (SCB->CCR & SCB_CCR_DC_Msk) park_error(V5_E_CACHE);
    g_mailbox.state = V5_INITIALIZING;
    ...
    if (info.n_inputs != 1 || info.n_outputs != 1 ||
        !tensor_ok(info.inputs,41,164) || !tensor_ok(info.outputs,5,20)) park_error(V5_E_LAYOUT);
    ...
    if (nin != 1 || nout != 1 || (uintptr_t)in[0] != V5_RUNTIME_IO_ADDRESS ||
        (uintptr_t)out[0] != V5_RUNTIME_IO_ADDRESS) park_error(V5_E_LAYOUT);
    ...
    g_mailbox.stage=0; __DMB(); g_mailbox.state=V5_READY; __DSB();
    uint32_t previous=0;
    for (;;) {
        uint32_t seq=g_mailbox.request_sequence;
        if (seq == previous) { __NOP(); continue; }
        if (seq != previous + 1 || seq == 0) park_error(V5_E_CHANGED);
        if (g_mailbox.platform_ack != V5_PLATFORM_ACK) park_error(V5_E_PLATFORM);
        if (g_mailbox.command != V5_COMMAND_INFER) park_error(V5_E_COMMAND);
        if (g_mailbox.input_count != 41) park_error(V5_E_INPUT);
        const uint32_t row=g_mailbox.row_id;
        g_mailbox.output_count=0; g_mailbox.run_cycles=0;
        g_mailbox.stage=5; g_mailbox.state=V5_RUNNING; __DMB();
        for (unsigned i=0; i<41; ++i) {
            input_copy[i]=g_mailbox.input_words[i];
            if (!finite_word(input_copy[i])) park_error(V5_E_INPUT);
        }
        __DMB();
        for (unsigned i=0; i<41; ++i)
            if (input_copy[i] != g_mailbox.input_words[i]) park_error(V5_E_CHANGED);
        if (g_mailbox.request_sequence != seq || g_mailbox.row_id != row) park_error(V5_E_CHANGED);
        ...
        for (unsigned i=0; i<5; ++i) if (!finite_word(output_copy[i])) park_error(V5_E_OUTPUT);
        if (g_mailbox.request_sequence != seq || g_mailbox.row_id != row) park_error(V5_E_CHANGED);
        g_mailbox.completed_row_id=row; g_mailbox.stage=0;
        previous=seq;
        __DMB(); g_mailbox.response_sequence=seq; g_mailbox.state=V5_DONE; __DSB();
    }
}
